...
export const sourceCodeItems: SourceCodeItem[] = [
    ...
    {
      id: 31,
      name: "iSparkTool — iOS All in One",
      description: "Premium iOS servicing tool (v6.9.4) covering A12+ bypass, ramdisk bypass, jailbreak bypass, MDM bypass, GSM signal restore, and full device info reading. Supports iOS 7–26.1 across iPhone 5s to iPhone X with no jailbreak required for some bypasses.",
      price: 250,
      originalPrice: 800,
      exclusivePrice: 800,
      ...
      rating: 4.9,
      downloads: 0,
      tags: ["C#", "iOS", "iCloud Bypass", "Ramdisk", "MDM", "GSM"],
      image: iSparkToolImage,
      badge: "new",
      features: [
        "iOS Version Support — iOS 7 through iOS 26.1; no jailbreak required for select bypass operations",
        "A12+ Bypass (iOS 26.0.1/26.1) — Activation lock bypass for A12 Bionic and newer chipsets on latest iOS versions",
        "Ramdisk Bypass (iOS 12–16) — Ramdisk-based iCloud bypass for iPhone 6 to iPhone X",
        "Jailbreak Bypass (iOS 12–17) — Bypass via jailbreak path for a wide range of supported devices",
        "MDM Bypass — Mobile Device Management profile removal for all supported iOS/iPhones/iPads",
        "GSM Signal Restore (iPhone 7–X, iOS 12–14.5) — Full cellular signal after bypass for GSM model iPhones",
        "Hello Bypass — Skip the initial Hello/setup screen on locked devices",
        "Passcode Backup & Passcode Activate — Backup device passcode and activate via passcode",
        "Patch USB — USB communication layer patching for device access",
        "Erase Device — Full device erasure from the tool interface",
        "Broken Baseband Bypass — Bypass for devices with broken or missing baseband hardware",
        "Enable / Disable Baseband — Baseband management for supported models",
        "Logout Account — Remove Apple ID / iCloud account from the device",
        "Device Detection — Detects devices in Normal, Recovery, and DFU modes automatically",
        "Full Device Info — Reads Serial Number, iOS Version, IMEI, ECID, UDID, Model Number, Activation State, Battery Level, Storage, Jailbreak status, and more",
        "Model Name Conversion — Converts product types (e.g. iPhone10,6) to human-readable names (iPhone X GSM)",
        "Purple Mode / ToolBox — Magic CFG, driver fixes, and advanced utilities via Purple.cs and ToolBox form",
        "Fix Drivers — One-click Apple driver repair for reliable device communication",
        "Driver Management — Full driver install/repair workflow via newShetouane.cs",
        "Device File Manager — Browse and manage on-device files (DeviceFileManager.cs)",
        "Telegram Notifications — Push operation notifications via Telegram bot (TelegramNotifier.cs)",
